...
| Chamados | `test-chamado-reasons.php` (motivo obrigatório e **estrutural**: os vocabulários de chamado e de devolução são disjuntos, senão um chamado geral autorizaria uma devolução), `test-chamado-authorization.php` (matriz de papéis, serialização por pedido e IDOR na criação), `test-chamado-close.php` (só loja e Papelito encerram; encerrado recusa mensagem), `test-message-rich-text.php` (nenhum HTML entra; `body` é derivado, não aceito), `test-whatsapp-url.php` (número real, `null` sem telefone, DDD 55 não perde o país), `test-vendor-messaging-transaction.php` (**estrutural**: criação de chamado e primeira mensagem numa transação só) |
| Devolução | `test-returns.php` (política pura e repasse do motivo do chamado), `test-return-support-message.php` (**mensagem canônica curta**: não repete pedido, loja, datas nem valores — eles vivem no painel de contexto) |
