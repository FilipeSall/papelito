	...
	static function (): void {
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => 'papelito_taxonomy_admin_permission',
				'callback'            => static function (): WP_REST_Response {
					return new WP_REST_Response( papelito_taxonomy_reconcile_report(), 200 );
				},
			)
		);
				...
				array(
					...
					'callback'            => static function ( WP_REST_Request $request ) {
						...
						if ( array_key_exists( 'categoryId', $body ) ) {
							$result = papelito_product_set_category( $product_id, (int) $body['categoryId'] );
						...
						if ( array_key_exists( 'subcategoryIds', $body ) ) {
							$result = papelito_product_set_subcategories( $product_id, papelito_taxonomy_id_list( $request, 'subcategoryIds' ) );
						...
						if ( array_key_exists( 'collections', $body ) ) {
							$slugs  = is_array( $body['collections'] ) ? $body['collections'] : array();
							$result = papelito_product_set_collections( $product_id, $slugs );
