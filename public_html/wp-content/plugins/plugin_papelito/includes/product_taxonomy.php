...
/**
 * Conta produtos publicados vinculados a uma categoria.
 *
 * @param int $category_id Id da categoria.
 * @return int
 */
function papelito_category_published_product_count( $category_id ) {
	global $wpdb;
	...
	return (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$wpdb->prepare(
			"SELECT COUNT(DISTINCT pc.product_id) FROM {$tables['product_category']} pc INNER JOIN {$wpdb->posts} p ON p.ID = pc.product_id AND p.post_type = 'product' AND p.post_status = 'publish' WHERE pc.category_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$category_id
		)
	);
}
...
function papelito_category_archive( $category_id ) {
	...
	$in_use = papelito_category_published_product_count( $category['id'] );
...
function papelito_product_set_category( $product_id, $category_id ) {
	...
	$current = papelito_product_get_category( $product_id );

	if ( null !== $current && $current['id'] === $category['id'] ) {
		return true;
	}
	...
	if ( false === $saved ) {
		...
		return new WP_Error( 'papelito_product_category_failed', 'Não foi possível gravar a categoria do produto.', array( 'status' => 500 ) );
	}
...
function papelito_product_set_subcategories( $product_id, array $subcategory_ids ) {
	...
	foreach ( $ids as $subcategory_id ) {
		...
		if ( $subcategory['categoryId'] !== $category['id'] ) {
			return new WP_Error(
				'papelito_subcategory_foreign',
				sprintf( 'A subcategoria "%s" não pertence à categoria "%s".', $subcategory['name'], $category['name'] ),
				array( 'status' => 422 )
			);
		}

		if ( ! $subcategory['isActive'] || null !== $subcategory['archivedAt'] ) {
			return new WP_Error( 'papelito_subcategory_inactive', 'Subcategoria inativa ou arquivada: ' . $subcategory['name'], array( 'status' => 422 ) );
		}
	...
	foreach ( $valid as $subcategory_id ) {
...
function papelito_category_integrity_report() {
	...
	$sql                    = "SELECT c.id AS category_id, c.name, COUNT(DISTINCT pc.product_id) AS published FROM {$tables['categories']} c INNER JOIN {$tables['product_category']} pc ON pc.category_id = c.id INNER JOIN {$wpdb->posts} p ON p.ID = pc.product_id AND p.post_type = 'product' AND p.post_status = 'publish' WHERE c.is_active = 0 OR c.archived_at IS NOT NULL GROUP BY c.id ORDER BY c.id ASC"; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
