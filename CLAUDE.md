...
- **O lint do editor é parte da entrega, e stub de teste não é exceção**: parâmetro tipado (`mixed` onde o core aceita qualquer coisa), corpo nunca vazio, `require_once` no include do módulo sob teste (`php:S2003`) e literal de fixture repetido em constante `<ESCOPO>_TEST_*`. Nenhum hint de SonarLint se cala com comentário — nem o `php:S1186`, que pede exatamente isso. Regras e forma canônica em [docs/context/testing.md](docs/context/testing.md#stub-que-nasce-limpo-no-editor).
- **Teste standalone novo não se escreve copiando o arquivo vizinho.** A maioria dos 185 scripts de `tests/` é anterior à regra de stub; copiar um deles reintroduz `php:S1186`, `php:S2003` e `P1132` em bloco. Comece pela forma canônica de [docs/context/testing.md](docs/context/testing.md#stub-que-nasce-limpo-no-editor), não pelo vizinho.
- **Campo novo em array alinhado realinha o array inteiro.** A chave mais longa define a coluna do `=>`. Acrescentar `'shipping_provider'` num array de chaves curtas sem reajustar as outras gera um `WordPress.Arrays.MultipleStatementAlignment` por linha e sobe o PHPCS de um arquivo que estava estável — foi assim que `papelito_vendor_dashboard_map_order()` ganhou 9 avisos de uma vez.
- **Ternário aninhado vira variável ou função** (`php:S3358`). O padrão `defined() ? CONST : ( function_exists() ? papelito_env() : getenv() )` é uma função com `return` antecipado — `papelito_shipping_provider_config()`; ternário passado como argumento de outro ternário é uma atribuição na linha de cima.
- **Callback REST não acumula complexidade** (`php:S3776`, teto 15). O callback registrado faz gate, delega e responde; ler o corpo da requisição, resolver preço e transformar linha são funções `papelito_<módulo>_<passo>` próprias. Closure dentro de `array_map` conta aninhamento duas vezes — extraia e passe o nome da função como callable. `papelito_shipping_quote_endpoint()` é a referência. Detalhe em [docs/context/architecture.md](docs/context/architecture.md#convenções-php).
