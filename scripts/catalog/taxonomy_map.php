 ...
 *
 * Subcategorias são facetas opcionais: a categoria principal basta para o
 * produto ser classificado e publicado no catálogo.
 *
...
function papelito_taxonomy_legacy_map() {
	return array(
		...
		// `Papel > Slim` é uma variante de formato, não material.
		'papel|slim'                    => array(
			'category'      => 'sedas',
...
function papelito_taxonomy_decisions() {
	return array(
		'D1' => 'Encerrada: material ausente não bloqueia a classificação de Sedas.',
		'D2' => 'Encerrada: "Insane" e "Pink" são linhas comerciais.',
		'D3' => 'Encerrada: acabamento e cor permanecem em pa_cor, não viram subcategoria.',
		'D4' => 'Encerrada: filtro sem tipo permanece só com a variante de tamanho conhecida.',
		'D5' => 'Encerrada: subcategoria de acessório é opcional.',
		'D8' => 'Encerrada: Slim é variante de formato, sem material implícito.',
	);
}
...
function papelito_taxonomy_resolve_destination( $title, array $root_slugs, array $child_slugs ) {
	...
	// Decisão confirmada: o SKU PP01010008 coloca esta Seda Insane na família
	// Tradicional. Pink continua apenas como linha porque não há evidência de material.
	if ( 'sedas' === $category && 0 === strcasecmp( trim( (string) $title ), 'Seda Insane King Size' ) ) {
		$subs[] = 'tradicional';
	}
	...
	if ( 'acessorios' === $category && array_intersect( $validas, array( 'dichavador', 'tubelito', 'bandeja-chaveiro' ) ) ) {
		$destino['notas'][] = 'acabamento/cor permanece em pa_cor; não é subcategoria.';
	}
