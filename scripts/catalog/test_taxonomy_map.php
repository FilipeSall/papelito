...
echo "\nPendências: o que o script se recusa a inventar\n";

$insane = papelito_taxonomy_resolve_destination( 'Seda Insane King Size', array( 'papel' ), array( 'premium' ) );
assert_map( 'Insane sem material fica pendente', 1, count( $insane['pendencias'] ) );
assert_map( 'a pendência cita a decisão D1', true, false !== strpos( $insane['pendencias'][0], 'D1' ) );

$slim = papelito_taxonomy_resolve_destination( 'Seda Slim King Size', array( 'papel' ), array( 'slim' ) );
assert_map( 'Seda Slim sem material cita D8, não D1', true, false !== strpos( $slim['pendencias'][0], 'D8' ) );

$filtro_slim = papelito_taxonomy_resolve_destination( 'Filtro Slim', array( 'filtro' ), array( 'slim-filtros' ) );
assert_map( 'Filtro sem tipo cita D4', true, false !== strpos( $filtro_slim['pendencias'][0], 'D4' ) );
