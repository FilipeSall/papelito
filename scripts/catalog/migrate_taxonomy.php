 ...
 * `_papelito_taxonomy_todo` e monta o mapa de equivalência para o dual-write.
 *
 * **Escreve SOMENTE nas tabelas `wp_papelito_*`.** `product_cat` não é tocado: o
 * dual-write fica suprimido durante a migração, de propósito.
 *
...
/**
 * Ids de termo de `product_cat` indexados por slug.
 *
 * @return array<string,int>
 */
function papelito_migration_legacy_term_ids() {
	global $wpdb;

	$sql  = "SELECT t.slug, t.term_id FROM {$wpdb->terms} t INNER JOIN {$wpdb->term_taxonomy} tt ON tt.term_id = t.term_id AND tt.taxonomy = 'product_cat'"; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	$rows = $wpdb->get_results( $sql, ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.NotPrepared
	$map  = array();
...
if ( $aplicar ) {
	// `product_cat` não pode ser tocado pela migração: a fase 4 escreve só nas
	// tabelas Papelito. Sem esta supressão, cada gravação disparia o dual-write.
	...
	$termos = papelito_migration_legacy_term_ids();
	$mapa   = array(
		'roots'       => array(),
		'children'    => array(),
		'collections' => array(),
	...
	foreach ( papelito_taxonomy_legacy_map() as $chave => $regra ) {
		list( $root_slug, $child_slug ) = array_pad( explode( '|', $chave, 2 ), 2, '' );

		if ( null !== $regra['category'] && isset( $termos[ $root_slug ] ) ) {
			$mapa['roots'][ $regra['category'] ] = $termos[ $root_slug ];
		}

		$subs = $regra['subcategories'] ?? array();

		// Entrada sem subcategoria não tem inversa confiável: `papel|` e
		// `papel|premium` produziriam a mesma chave e uma sobrescreveria a outra.
		if ( null !== $regra['category'] && '' !== $child_slug && ! empty( $subs ) && isset( $termos[ $child_slug ] ) ) {
			sort( $subs );
			$mapa['children'][ $regra['category'] . '|' . implode( ',', $subs ) ] = $termos[ $child_slug ];
		}

		foreach ( $regra['collections'] ?? array() as $collection ) {
			if ( isset( $termos[ $child_slug ] ) ) {
				$mapa['collections'][ $collection ] = $termos[ $child_slug ];
			} elseif ( isset( $termos[ $root_slug ] ) ) {
				$mapa['collections'][ $collection ] = $termos[ $root_slug ];
			}
		}
	}

	foreach ( papelito_taxonomy_legacy_inverse_hints() as $chave => $slug_legado ) {
		if ( isset( $termos[ $slug_legado ] ) ) {
			$mapa['children'][ $chave ] = $termos[ $slug_legado ];
		}
	}

	papelito_taxonomy_set_legacy_term_map( $mapa );
...
if ( $aplicar ) {
	...
	$mapa_gravado = papelito_taxonomy_legacy_term_map();

	WP_CLI::log( sprintf( '  mapa de equivalência: %d raiz(es), %d filho(s), %d coleção(ões)', count( $mapa_gravado['roots'] ), count( $mapa_gravado['children'] ), count( $mapa_gravado['collections'] ) ) );
	...
	$reconciliacao = papelito_taxonomy_reconcile_report();

	WP_CLI::log( '' );
	WP_CLI::log( sprintf( '  flag PAPELITO_PRODUCT_TAXONOMY_ENABLED: %s', $reconciliacao['flagEnabled'] ? 'ligada' : 'desligada' ) );
	WP_CLI::log(
		sprintf(
			'  reconciliação com product_cat: %d conferidos, %d divergentes, %d COM PERDA de termo',
			$reconciliacao['conferidos'],
			count( $reconciliacao['divergentes'] ),
			count( $reconciliacao['comPerda'] )
		)
	);

	foreach ( $reconciliacao['divergentes'] as $d ) {
		WP_CLI::log(
			sprintf(
				'    #%d %s termo(s): %s',
				$d['productId'],
				empty( $d['perdidos'] ) ? 'ganharia' : 'PERDERIA',
				implode( ', ', empty( $d['perdidos'] ) ? $d['adicionados'] : $d['perdidos'] )
			)
		);
	}
}
